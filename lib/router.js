...
Router.route('/player-profile', function () {
    ...
    data: function () {
       return {
        ...
        playerVideos: UploadPlayerVideos.find({owner: Meteor.userId()}).fetch()
    }
}
});
...
Router.route('/view-player-details/:_id', function () {
    ...
    data:function () {
        ...
        return{
            ...
            viewPlayerVideo: UploadPlayerVideos.find({myId: _id}).fetch()
...
Router.route('/club-profile', function () {
    ...
    data: function () {
        return{
            clubProfile: Clubs.findOne({owner: Meteor.userId()}),
            clubVideos: UploadClubVideos.find({owner: Meteor.userId()}).fetch()
...
Router.route('/view-club-details/:_id', function () {
    ...
    data:function () {
        ...
        return{
            ...
            viewClubVideo: UploadClubVideos.find({myId: _id}).fetch()
...
//
//Router.route('/this-video/:_id', function () {
...
//        return{
//            playerProfile: Players.findOne({owner: Meteor.userId()}),
//            oneVideo: UploadVideos.findOne(_id),
//            playerVideos: UploadVideos.find({owner: Meteor.userId()}).fetch()
//
//
//        }
...
Router.route('/player-videos/:_id', function () {
    this.render("playerVideosPage");
},{

    name: "playerVideosPage",
    data:function () {
        var _id = this.params._id;
        return{
            playerProfile: Players.findOne({owner: Meteor.userId()}),
            onePlayerVideo: UploadPlayerVideos.findOne(_id),
            playerVideos: UploadPlayerVideos.find({owner: Meteor.userId()}).fetch()
...
Router.route('/club-videos/:_id', function () {
    this.render("clubVideosPage");
},{
    ...
    data:function () {
        var _id = this.params._id;
        return{
            clubProfile: Players.findOne({owner: Meteor.userId()}),
            oneClubVideo: UploadClubVideos.findOne(_id),
            clubVideos: UploadClubVideos.find({owner: Meteor.userId()}).fetch()
...
Router.route('/new-player-video', function () {
    this.render("uploadPlayerVideos");
},{
    name: "uploadPlayerVideos",
    data: function () {

            return {
                showallvideos: UploadPlayerVideos.find().fetch()
            }
        }

});

Router.route('/new-club-video', function () {
    this.render("uploadClubVideos");
},{
    name: "uploadClubVideos",
    data: function () {
        return {
            showallclubvideos: UploadClubVideos.find().fetch()
        }
    }
});